...
export default {
  ...
  uploadFile: async (body) => {
    ...
    let url =
      "https://zenodo.org/api/deposit/depositions/"+body.id_zenodo+"/files?access_token=UkO33J14iE8Svd4Ck4VvfT4BDuT25uwY0zwdRXiWIPHOr3iRJbegI7rc8Emh";
    if (token.length > 0 && body.id_zenodo > 0) {
      url =
        "https://zenodo.org/api/deposit/depositions/" +
        body.id_zenodo +
        "/files?access_token=" +token;
    }
    const data = await axios
      .post(url, form)
      .then(function (response) {
  ...
  publishZenodoRepo: async (body) => {
    let tokenAPI = body.token.token
    let url =
      "https://zenodo.org/api/deposit/depositions/"+body.id_zenodo+"/actions/publish?access_token=UkO33J14iE8Svd4Ck4VvfT4BDuT25uwY0zwdRXiWIPHOr3iRJbegI7rc8Emh";
